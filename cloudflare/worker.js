...
const CORS_HEADERS = {
  ...
  'access-control-allow-headers': 'content-type,authorization,x-taif-app-key'
};
...
function normalizeDomainStatePayloadForStorage(key, incomingPayload, existingPayload = null) {
  ...
  if (RECORD_STATE_KEYS.has(safeKey)) {
...
export default {
  async fetch(request, env) {
    try {
      ...
      if (!match || request.method !== 'POST') return json({ ok: false, message: 'Not found' }, 404);
      ...
      const result = await dispatchRpc(env.DB, name, payload || {}, request);
      return json(result ?? { ok: true });
    } catch (error) {
      const status = Number(error?.status) || 500;
      return json({ ok: false, message: error?.message || 'تعذر تنفيذ العملية.', code: error?.code || '' }, status);
    }
  }
};
...
async function insertAudit(db, { workspaceId: wid = DEFAULT_WORKSPACE_ID, actor, action, entityType, entityId, details }) {
  const at = nowIso();
...
async function publicPriceDisplayState(db, p) {
  ...
  const key = text(p.p_state_key || p.stateKey || p.state_key || DOMAIN_KEYS.currency) || DOMAIN_KEYS.currency;
  if (key !== DOMAIN_KEYS.currency) fail('مفتاح شاشة العرض غير مسموح.', 403, 'PUBLIC_STATE_KEY_NOT_ALLOWED');
  const row = await db.prepare('select * from taif_domain_state where workspace_id = ? and state_key = ? limit 1').bind(wid, key).first();
  if (!row) return { ok: true, workspaceId: wid, workspace_id: wid, stateKey: key, state_key: key, state: null, payload: null };
  const state = stateToJson(row);
  const payload = state && state.payload && typeof state.payload === 'object' && !Array.isArray(state.payload) ? state.payload : {};
  const publicState = {
    ...payload,
    __taifDisplayRevision: Number(state.revision) || 0,
    __taifDomainRevision: Number(state.revision) || 0,
    __taifDisplayPayloadHash: text(state.payloadHash || state.payload_hash),
    __taifDisplayUpdatedAt: text(state.updatedAt || state.updated_at || state.logicalUpdatedAt || state.logical_updated_at),
    revision: Number(state.revision) || 0,
    payloadHash: text(state.payloadHash || state.payload_hash),
    updatedAt: text(state.updatedAt || state.updated_at || state.logicalUpdatedAt || state.logical_updated_at)
  };
  return {
    ok: true,
    workspaceId: wid,
    ...
    revision: Number(state.revision) || 0,
    payloadHash: text(state.payloadHash || state.payload_hash),
    payload_hash: text(state.payloadHash || state.payload_hash),
    updatedAt: publicState.updatedAt,
    updated_at: publicState.updatedAt,
    state: publicState,
    payload: publicState
  };
}
