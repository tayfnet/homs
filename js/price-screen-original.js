(()=>{
  ...
  function getDisplayCurrencyCode(row){
    return normalizeCurrencyCode(row && row.code) || '—';
  }
  ...
  function mobileRowMarkup(row, counterpart){
    const isCounterpartCurrency = isCounterpartCurrencyRow(row, counterpart);
    const rowFlag = resolveRowFlagAsset(row, 'circle');
    const rowFlagTitle = row.name || row.code;

    return `
      <div class="price-screen__row-shell">
        <div class="price-screen__row${row.isUsd ? ' price-screen__row--usd' : ''}${isCounterpartCurrency ? ' price-screen__row--counterpart' : ''}" data-code="${escapeHtml(row.code)}" ${isCounterpartCurrency ? 'data-price-screen-counterpart-card="true" role="button" tabindex="0" aria-label="تعديل سعر عملة التسعير" title="تعديل سعر عملة التسعير"' : ''}>
          <div class="price-screen__code" dir="ltr">${safeControlText(getDisplayCurrencyCode(row))}</div>
          <div class="price-screen__name">
            <div class="price-screen__name-main">${escapeHtml(row.name || row.code)}</div>
          </div>
          <div class="price-screen__num price-screen__num--buy">${safeControlText(formatRowCellValue(row, 'buy', 6))}</div>
          <div class="price-screen__flag" title="${escapeHtml(rowFlagTitle)}" aria-hidden="true">
            <img class="price-screen__flag-image" src="${escapeHtml(rowFlag.src)}" alt="" draggable="false" loading="eager" decoding="async" width="512" height="512">
          </div>
          <div class="price-screen__num price-screen__num--sell">${safeControlText(formatRowCellValue(row, 'sell', 6))}</div>
        </div>
      </div>
    `;
  }

  function renderMobilePriceTable(source){
    const rows = getVisibleRows(source.rows);
    return `
      <section class="price-screen price-screen--mobile-table price-screen--display-only" aria-label="شاشة الأسعار للموبايل">
        <div class="price-screen__board-head" aria-hidden="true">
          <div class="price-screen__board-head-main price-screen__board-head-main--mobile">
            <div>${createSingleLineHeaderLabel('الكود', { className: 'taif-singleline-fit--price-head taif-singleline-fit--price-head-mobile', min: 11.2, max: 13.2, minScale: .95 })}</div>
            <div class="price-screen__board-head-name">${createSingleLineHeaderLabel('اسم العملة', { className: 'taif-singleline-fit--price-head taif-singleline-fit--price-head-name taif-singleline-fit--price-head-mobile', min: 11.4, max: 13.4, minScale: .95 })}</div>
            <div>${createSingleLineHeaderLabel('شراء', { className: 'taif-singleline-fit--price-head taif-singleline-fit--price-head-mobile', min: 11.2, max: 13.2, minScale: .95 })}</div>
            <div class="price-screen__board-head-middle-note">${createSingleLineHeaderLabel('(مقابل الليرة السورية)', { className: 'taif-singleline-fit--price-head taif-singleline-fit--price-head-mobile taif-singleline-fit--price-head-middle-note', min: 7.2, max: 8.8, minScale: .88 })}</div>
            <div>${createSingleLineHeaderLabel('مبيع', { className: 'taif-singleline-fit--price-head taif-singleline-fit--price-head-mobile', min: 11.2, max: 13.2, minScale: .95 })}</div>
          </div>
        </div>
        <div class="price-screen__board-body" aria-label="جدول الأسعار">
          ${rows.length ? `<div class="price-screen__list">${rows.map((row) => mobileRowMarkup(row, source.counterpart)).join('')}</div>` : `<div class="price-screen__empty">لا توجد عملات متاحة للعرض حاليًا.</div>`}
        </div>
      </section>
    `;
  }
