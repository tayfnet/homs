(()=>{
  ...
  function runCleanupSlot(target, key){
    ...
    if(typeof cleanup === 'function'){
      try{ cleanup(); }catch(error){ console.warn('[TAIF public display] cleanup failed', error); }
    }
  }
  ...
  function runCleanupCallbacks(callbacks){
    ...
    callbacks.splice(0).forEach((callback) => {
      if(typeof callback === 'function'){
        try{ callback(); }catch(error){ console.warn('[TAIF public display] cleanup callback failed', error); }
      }
    });
  }
  ...
  // read-only in-memory public state pulled from Cloudflare Worker + D1.
  // Nothing is written back to the main project from this display.
  ...
  function emit(name, detail){
    ...
    if(set){
      Array.from(set).forEach((listener) => {
        try{ listener(detail); }catch(error){ console.warn('[TAIF public display] listener failed', error); }
      });
    }
