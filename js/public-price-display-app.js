(()=>{
  ...
  function getNetworkStatusGraceMs(){
    const pollMs = Math.max(800, Number(config.POLL_INTERVAL_MS || 1200));
    return Math.max(25000, pollMs * 12);
  }

  function hasFreshDisplayedPrices(){
    return Boolean(initialized && lastAppliedAt && Date.now() - Number(lastAppliedAt) < getNetworkStatusGraceMs());
  }

  function showNetworkStatusWhenReallyNeeded(message){
    // لا نُظهر تحذير انقطاع الاتصال فوق أسعار حديثة بسبب فشل عابر أو مهلة RPC قصيرة.
    // يظهر التحذير فقط عند انقطاع المتصفح فعليًا أو عند عدم وجود أسعار حديثة معروضة.
    if(isNativeOffline() || !hasFreshDisplayedPrices()) showStatus(message, true);
    else showStatus('');
  }
  ...
  async function refresh(reason = 'poll'){
    if(stopped) return;
    if(isNetworkPaused()){
      if(reason !== 'poll' || consecutiveFailures >= 1){
        showNetworkStatusWhenReallyNeeded('الاتصال غير متوفر، سيتم تحديث الأسعار تلقائيًا عند عودة الاتصال');
      }
      return;
    }
    if(refreshInFlight){
